...
| 10  | Class context label                            | `@LogContext(name)` (class decorator — records label; `setContext()` applies it) + `LOG_CONTEXT_METADATA_KEY`                                   | `downstream/downstream.service.ts`; `library-probe.ts` (`LOG_CONTEXT_METADATA_KEY`)                        | ✅     |
...
| 34  | Pretty dev destination                         | `PrettyDevDestination` / `isPretty`                                                                                                             | dev mode via `isPretty`; `library-probe.ts` references the `PrettyDevDestination` class                    | ✅     |
...
| 41  | Isomorphic `/shared` types                     | `LogLevel`, `LogEntry`, `ServiceMetadata`, `ReservedLogKey`                                                                                     | `apps/web` form types + `PrismaLogDestination` typing; `library-probe.ts` (`ReservedLogKey`)               | ✅     |
...
| 45b | `otel` per-field name overrides                | `OtelOptions`: `otel.traceIdField` / `spanIdField` / `traceFlagsField`                                                                          | `apps/worker` sets `traceIdField: 'trace_id'` (§14); `OtelOptions` typed in `apps/api` `logger.config.ts`  | ✅     |
...
| 45e | Remaining injection tokens                     | `LOGGER_PINO_INSTANCE_TOKEN` / `LOGGER_DESTINATIONS_TOKEN` / `LOG_CONTEXT_TOKEN`                                                                | `apps/api/src/library-probe.ts` (token-resolution proof)                                                   | ✅     |
...
> **Coverage rule.** Every public export from `@bymax-one/nest-logger` (server `.` subpath and `./shared` subpath) is referenced from at least one file in this repository. The CI step `scripts/audit-library-exports.mjs` (run via `pnpm audit:exports`) parses the shipped declarations at `dist/server/index.d.ts` and `dist/shared/index.d.ts`, extracts every exported symbol, and word-boundary-searches the `apps/**` corpus, failing the build if any export is unused. Genuinely-internal symbols that leak into the published `.d.ts` may be allow-listed in `.audit-ignore.json` with a reason — never to silence a demonstrable export. The matrix above is reconciled against that script.
