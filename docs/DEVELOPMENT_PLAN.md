...
>
> **Overall progress: 112 / 133 tasks done (84%)**
...
| 13  | Dashboard — Trigger, Alerts, Maintenance  | `phase-13-dashboard-ops.md`       | 9 / 9        | 100% | 🟢     |
...
- [x] `app/trigger/page.tsx` — fire every log type/feature; auto-pivot Explorer to the new `requestId`/`traceId`.
- [x] `app/alerts/page.tsx` — rule form (`expr + threshold + for`), channel registry, incident timeline.
- [x] `app/maintenance/page.tsx` — retention sweep status, JSON/CSV export, RBAC role/tenant, **redaction-at-source** proof panel, audit table.
...
- [x] `apps/api` Jest unit (ESM) — `coverageThreshold.global` = `{ branches, lines, functions, statements }: 100`; `collectCoverageFrom` excludes `*.spec`/`*.module`/`main.ts`/`*.dto`/`*.d.ts`.
- [x] `apps/api` supertest e2e — **stdout-capture** assertions (spy `process.stdout.write`): logKeys, URL norm, `requestId`, `[REDACTED]`, double-log avoidance; `logs/` API paging/aggregate/facets/SSE; `apps/worker` traceId propagation.
- [x] `apps/web` Vitest (`jsdom`, v8) — `lib/**` + `components/**` thresholds all **100**; Playwright journeys (fire → live Explorer → trace; brush → filter; RBAC scoping) — journeys run against a live stack.
- [x] Optional `docker-compose.test.yml` + `infra:test:up/down` + Testcontainers Loki (`test:int`) — gated out of the hermetic default runs; requires Docker.
